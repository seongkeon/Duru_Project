...
<c:if test="${!empty tList}">
    <c:forEach items="${tList}" var="tax">
        <tr id="taxRow-${tax.tid}" style="${tax.status == 1 ? 'display : none;' : 'display : table-row'}">
        ...
        <td>
          <input type="button" id="tBook-btn" value="호출"
                 onclick="tBook('${tax.tid}', '${tax.status}')">
        </td>
      </tr>
    </c:forEach>
</c:if>
...
<script>
  ...
  function tBook(tid, status) {
    console.log(tid);
    alert("택시를 호출하였습니다.")
    // 택시의 상태(status)가 0인 경우에만 호출 버튼 숨김 처리
    if (status === "0") {
      const row = document.getElementById(`taxRow-${tid}`);
      if (row) {
